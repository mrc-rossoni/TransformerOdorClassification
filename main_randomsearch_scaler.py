...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------
    # ---- Standardise features (fit on train, transform everywhere) ----
    # ------------------------------------------------------------------
    scaler = StandardScaler()

    # Flatten to 2-D so sklearn sees “(samples × features)”
    train_flat = train_features.reshape(-1, 1)
    val_flat   =  val_features.reshape(-1, 1)
    test_flat  = test_features.reshape(-1, 1)

    train_scaled = scaler.fit_transform(train_flat).reshape(train_features.shape)
    val_scaled   = scaler.transform(val_flat).reshape(val_features.shape)
    test_scaled  = scaler.transform(test_flat).reshape(test_features.shape)

    # Convert to PyTorch tensors
    train_features = torch.tensor(train_scaled, dtype=torch.float32)
    val_features   = torch.tensor(val_scaled,   dtype=torch.float32)
    ...
    for i in tqdm(range(10), desc="Random Search Progress"):
        config = {
            'Net_Type': 'C-T',
            ...
            'emb_size': random.choice(hyperparam_space['emb_size']),
            'num_heads': random.choice(hyperparam_space['num_heads']),
            'dim_ff': random.choice(hyperparam_space['dim_ff']),
            'epochs': random.choice(hyperparam_space['epochs']),
        ...
        for epoch in range(config['epochs']):
            ...
            val_metrics, _ = val_trainer.evaluate()
    ...
    train_loader = DataLoader(train_dataset, batch_size=best_config['batch_size'], shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=best_config['batch_size'], shuffle=False, num_workers=4, pin_memory=True)
    test_loader = DataLoader(test_dataset, batch_size=best_config['batch_size'], num_workers=4, pin_memory=True)

    optimizer_class = get_optimizer("Adam")
    optimizer = optimizer_class(best_model.parameters(), lr=best_config['lr'])
    loss_module = get_loss_module()

    trainer = SupervisedTrainer(best_model, train_loader, device, loss_module, optimizer, print_conf_mat=True)
    ...
    for epoch in range(best_config['epochs']):
        ...
        print(f"Epoch {epoch+1}/{best_config['epochs']} - Training Loss: {train_loss:.4f} - Training Accuracy: {train_accuracy:.4f}")

        val_trainer = SupervisedTrainer(best_model, val_loader, device, loss_module)
        ...
        print(f"Epoch {epoch+1}/{best_config['epochs']} - Validation Loss: {val_metrics_epoch['loss']:.4f} - Validation Accuracy: {val_metrics_epoch['accuracy']:.4f}")

    test_trainer = SupervisedTrainer(best_model, test_loader, device, loss_module)
