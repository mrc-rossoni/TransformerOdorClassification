...
# Disabilita gli avvisi di UndefinedMetricWarning
...
class SupervisedTrainer(BaseTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(args[3], torch.nn.CrossEntropyLoss):
            self.analyzer = analysis.Analyzer(print_conf_mat=kwargs.get('print_conf_mat', False))
        else:
    ...
    def train_epoch(self, epoch_num=None):
        ...
        correct_predictions = 0  # correct predictions in epoch

        for i, batch in enumerate(self.dataloader):
            X, targets = batch
            X, targets = X.to(self.device), targets.to(self.device)  # Sposta i dati sulla GPU
        ...
        epoch_accuracy = correct_predictions / total_samples  # accuracy for whole epoch
        self.epoch_metrics['epoch'] = epoch_num
        self.epoch_metrics['loss'] = epoch_loss
        self.epoch_metrics['accuracy'] = epoch_accuracy
        return epoch_loss, epoch_accuracy  # Return numeric values for loss and accuracy
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        self.epoch_metrics['precision'] = precision_score(targets, predictions, average='macro', zero_division=0)  # average precision over all classes

        # Calcola il punteggio F1 macro
        macro_f1_score = f1_score(targets, predictions, average='macro', zero_division=0)
        self.epoch_metrics['macro_f1_score'] = macro_f1_score

        return self.epoch_metrics, {'targets': targets, 'predictions': predictions}  # Return metrics and details
