...
class ConvTran(nn.Module):
    def __init__(self, config, num_classes):
        ...
        # Embedding Layer -----------------------------------------------------------
        self.embed_layer = None  # Initialize as None, will be set in forward method
        self.embed_layer2 = nn.Sequential(
            nn.Conv2d(emb_size * 4, emb_size, kernel_size=[1, 1], padding='valid'),
    ...
    def forward(self, x):
        ...
        if self.embed_layer is None:
            self.embed_layer = nn.Sequential(
                nn.Conv2d(input_channels, self.emb_size * 4, kernel_size=[1, 8], padding='same'),
                nn.BatchNorm2d(self.emb_size * 4),
                nn.GELU()
            ).to(x.device)

        x = x.unsqueeze(1)  # Add a channel dimension
        x = x.unsqueeze(2)  # Add a height dimension to make it 4D
